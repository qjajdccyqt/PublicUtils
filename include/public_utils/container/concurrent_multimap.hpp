...
{
    typedef std::multimap<KeyType, typename std::conditional<shared, std::shared_ptr<ValueType>, ValueType>::type> DataType;
public:
...
class ConcurrentMultiMap<KeyType, ValueType, true>
{
    typedef std::multimap<KeyType, typename std::shared_ptr<ValueType>> DataType;
public:
    std::vector<typename DataType::mapped_type> Get(const KeyType& key)
    {
        std::vector<typename DataType::mapped_type> result;
